...
(function () {
  ...
  enum PER_DATA_DIR { ROWS_MODE, COLUMNS_MODE };

  type HemetologyMetaData = {
    tableQuery: string;
    labelIndex: number;
    valueIndex: number;
    dataAlignMode: PER_DATA_DIR;
  }
  ...
  function rejudgeItemColor(labelCell: Element, valueCell: Element) {
  ...
  function extractItems(tableName: string, meta: HemetologyMetaData) {
  ...
  function main() {
    ...
    extractItems('Table for Moblie', { tableQuery: table4mobile, labelIndex: 2, valueIndex: 3, dataAlignMode: PER_DATA_DIR.COLUMNS_MODE });
    extractItems('Table for Desktop', { tableQuery: table4Desktop, labelIndex: 2, valueIndex: 3, dataAlignMode: PER_DATA_DIR.COLUMNS_MODE });

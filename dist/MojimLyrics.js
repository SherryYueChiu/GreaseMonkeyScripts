...
(function () {
    ...
    let curLyricIndex = 21; // 記錄當前播放的歌詞索引
    ...
    // 取得歌詞內容的函數
    function getLyrics() {
        const lyricsSrc = Array.from(document.querySelectorAll('table tr')[1]
            .querySelector('td').innerText
            .split('\n').filter(_ => _.match(/\[[\d\:\.]+\]/)));
        let lyrics = [];
        for (let sentences of lyricsSrc) {
            const times = Array.from(sentences.matchAll(/\[(\d+\:\d+\.\d+)\]/g)).map(_ => _[1]);

...
(function () {
    const crawler = () => {
        ...
        for (let i = 0; i < classCnt; i++) {
            let ccode = '', cname = '', croom = [], ctime = [];
            // 選課代碼
            ccode = classBlock[i].querySelector('[data-title=選課代碼]')?.textContent?.trim() || '';
            // 課程名稱
            const courseNameElement = classBlock[i].querySelector('[data-title=課程名稱]');
            if (courseNameElement?.innerHTML.includes("strike"))
                continue;
            else {
                cname = courseNameElement?.textContent?.trim() || '';
            }
            // 地點
            const roomText = classBlock[i].querySelector("[data-title=時間地點]")?.textContent
                ?.replace('無資料', '')
                .trim()
                .match(/\[([^\[\]]*)\]/g);
            if (roomText) {

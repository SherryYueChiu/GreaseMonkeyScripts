...
(async function () {
    ...
    let game_mushroom = () => {
        ...
        if (timesLeft !== 0)
            collectMushroom(timesLeft);
        console.log("done");
    };
    /** 城市PK */
    let game_cityPK = async () => {
        window.showWindow('hux_city', 'plugin.php?id=hux_city:hux_city&mod=sign', 'get', 0);
        ...
        if (document.querySelector('#hcwindow form').innerHTML.indexOf("未簽到") != -1) {
            ...
            window.hideWindow('hux_city');
    };
    // main
    const urlParams = new URLSearchParams(window.location.search);
    const pageId = urlParams.get('id');
    if (pageId === "yinxingfei_zzza:yinxingfei_zzza_hall") {
        if (GM_getValue('game_dailyDraw') != getDate())
            game_dailyDraw();
    }
        ...
        if (GM_getValue('game_cityPK') != getDate())
            game_cityPK();
    }
})();
